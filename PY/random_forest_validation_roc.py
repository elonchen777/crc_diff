from pathlib import Path

import matplotlib.pyplot as plt
import numpy as np
import pandas as pd
import seaborn as sns
from sklearn.ensemble import RandomForestClassifier
from sklearn.feature_selection import SelectKBest, f_classif
from sklearn.metrics import auc, classification_report, confusion_matrix, roc_curve
from sklearn.model_selection import train_test_split
from sklearn.preprocessing import label_binarize

from dataset import BioSmokeDataset
from split_group import prepare_crc_diff_groups


GROUP_MAP = {
    'CTRL': 'CTRL',
    'CRC_welldiff': 'CRC-Well',
    'CRC_poordiff': 'CRC-Poor',
}

LABEL_MAP = {
    'CTRL': 0,
    'CRC-Well': 1,
    'CRC-Poor': 2,
}

CLASS_ORDER = ['CTRL', 'CRC-Well', 'CRC-Poor']
CLASS_COLORS = {
    'CTRL': '#2E86AB',
    'CRC-Well': '#F18F01',
    'CRC-Poor': '#D7263D',
}


def clean_feature_name(feature_name: str) -> str:
    if feature_name.startswith('tax_'):
        return feature_name.replace('tax_', '', 1)
    if feature_name.startswith('met_'):
        return feature_name.replace('met_', '', 1)
    return feature_name


def feature_type(feature_name: str) -> str:
    if feature_name.startswith('tax_'):
        return 'Microbe'
    if feature_name.startswith('met_'):
        return 'Metabolite'
    return 'Other'


def load_analysis_data() -> pd.DataFrame:
    ds = BioSmokeDataset()
    ds.preprocess_taxonomy_data(transform=True)
    ds.preprocess_metabolomics_data(relative_abund=False, transform=True)
    merged = ds.intersection_to_dataframe()

    grouped = prepare_crc_diff_groups(merged)
    grouped['group'] = grouped['group'].map(GROUP_MAP)
    grouped = grouped[grouped['group'].isin(CLASS_ORDER)].copy()

    return grouped


def prepare_features(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.Series]:
    feature_cols = [
        column for column in df.columns
        if column.startswith('tax_') or column.startswith('met_')
    ]
    if not feature_cols:
        raise ValueError('未找到 tax_ 或 met_ 特征列。')

    x = df[feature_cols].apply(pd.to_numeric, errors='coerce').fillna(0.0)
    x = x.loc[:, x.var(axis=0) > 0].copy()
    y = df['group'].map(LABEL_MAP)

    if y.isna().any():
        raise ValueError('存在无法映射到标签编码的 group。')

    return x, y.astype(int)


def select_feature_count(feature_count: int) -> int:
    return max(20, min(30, feature_count)) if feature_count >= 20 else feature_count


def train_model(x_train: pd.DataFrame, y_train: pd.Series) -> dict:
    """Train random forest model on training data"""
    selected_feature_count = select_feature_count(x_train.shape[1])
    selector = SelectKBest(score_func=f_classif, k=selected_feature_count)
    forest = RandomForestClassifier(
        n_estimators=1000,
        random_state=42,
        class_weight='balanced_subsample',
        min_samples_leaf=2,
        n_jobs=-1,
    )

    x_train_selected = selector.fit_transform(x_train, y_train)
    forest.fit(x_train_selected, y_train)

    selected_columns = x_train.columns[selector.get_support()].tolist()
    importance_df = pd.DataFrame(
        {
            'feature': selected_columns,
            'importance': forest.feature_importances_,
        }
    ).sort_values('importance', ascending=False)
    importance_df['feature_type'] = importance_df['feature'].map(feature_type)
    importance_df['display_name'] = importance_df['feature'].map(clean_feature_name)

    return {
        'selector': selector,
        'forest': forest,
        'importance_df': importance_df,
        'selected_columns': selected_columns,
    }


def plot_validation_roc_curves(
    y_val: pd.Series,
    val_proba: np.ndarray,
    output_dir: Path,
) -> pd.DataFrame:
    """Plot ROC curves for validation cohort"""
    y_binary = label_binarize(y_val, classes=[0, 1, 2])
    roc_rows = []

    plt.figure(figsize=(8, 7))
    for class_index, class_name in enumerate(CLASS_ORDER):
        fpr, tpr, _ = roc_curve(y_binary[:, class_index], val_proba[:, class_index])
        roc_auc = auc(fpr, tpr)
        roc_rows.append({'class': class_name, 'auc': roc_auc})
        plt.plot(
            fpr,
            tpr,
            color=CLASS_COLORS[class_name],
            linewidth=2.2,
            label=f'{class_name} vs rest (AUC = {roc_auc:.3f})',
        )

    plt.plot([0, 1], [0, 1], linestyle='--', color='#888888', linewidth=1)
    plt.xlim(0, 1)
    plt.ylim(0, 1.02)
    plt.xlabel('False Positive Rate')
    plt.ylabel('True Positive Rate')
    plt.title('Validation Cohort - One-vs-Rest ROC Curves')
    plt.legend(loc='lower right')
    plt.tight_layout()
    plt.savefig(output_dir / 'validation_roc_curves_ovr.png', dpi=300, bbox_inches='tight')
    plt.close()

    return pd.DataFrame(roc_rows)


def plot_validation_confusion(y_val: pd.Series, val_pred: np.ndarray, output_dir: Path) -> pd.DataFrame:
    """Plot confusion matrix for validation cohort"""
    cm = confusion_matrix(y_val, val_pred, labels=[0, 1, 2])
    cm_df = pd.DataFrame(cm, index=CLASS_ORDER, columns=CLASS_ORDER)

    plt.figure(figsize=(7, 6))
    sns.heatmap(cm_df, annot=True, fmt='d', cmap='Blues', cbar=False)
    plt.xlabel('Predicted Label')
    plt.ylabel('True Label')
    plt.title('Validation Cohort - Confusion Matrix')
    plt.tight_layout()
    plt.savefig(output_dir / 'validation_confusion_matrix.png', dpi=300, bbox_inches='tight')
    plt.close()

    return cm_df


def plot_feature_importance(importance_df: pd.DataFrame, output_dir: Path) -> pd.DataFrame:
    """Plot top 20 feature importance"""
    top20 = importance_df.head(20).copy()
    top20 = top20.iloc[::-1]

    color_map = {'Microbe': '#287271', 'Metabolite': '#E76F51', 'Other': '#666666'}
    colors = top20['feature_type'].map(color_map)

    plt.figure(figsize=(11, 8))
    plt.barh(top20['display_name'], top20['importance'], color=colors)
    plt.xlabel('Random Forest Importance')
    plt.ylabel('Feature')
    plt.title('Top 20 Feature Importance (Training Set)')
    plt.tight_layout()
    plt.savefig(output_dir / 'validation_feature_importance_top20.png', dpi=300, bbox_inches='tight')
    plt.close()

    return top20.iloc[::-1]


def save_validation_reports(
    output_dir: Path,
    df_train: pd.DataFrame,
    df_val: pd.DataFrame,
    y_val: pd.Series,
    val_pred: np.ndarray,
    val_proba: np.ndarray,
    model_results: dict,
    roc_df: pd.DataFrame,
    confusion_df: pd.DataFrame,
) -> None:
    """Save validation results and statistics"""
    importance_df = model_results['importance_df']
    biomarkers_df = importance_df.head(10).copy()
    biomarkers_df['label_code'] = biomarkers_df['feature'].map(lambda _: '')

    top20_df = importance_df.head(20).copy()
    top20_df.to_csv(output_dir / 'validation_feature_importance_top20.csv', index=False)
    importance_df.to_csv(output_dir / 'validation_feature_importance_all_selected.csv', index=False)
    biomarkers_df.to_csv(output_dir / 'validation_biomarker_panel_top10.csv', index=False)
    roc_df.to_csv(output_dir / 'validation_roc_auc_summary.csv', index=False)
    confusion_df.to_csv(output_dir / 'validation_confusion_matrix.csv')

    # Save validation predictions
    metadata_df = df_val[['group']].copy()
    metadata_df['label_code'] = y_val.values
    metadata_df['predicted_label_code'] = val_pred
    metadata_df['predicted_group'] = metadata_df['predicted_label_code'].map({v: k for k, v in LABEL_MAP.items()})
    proba_df = pd.DataFrame(
        val_proba,
        index=df_val.index,
        columns=[f'prob_{class_name}' for class_name in CLASS_ORDER],
    )
    pd.concat([metadata_df, proba_df], axis=1).to_csv(output_dir / 'validation_predictions.csv')

    report_text = classification_report(
        y_val,
        val_pred,
        labels=[0, 1, 2],
        target_names=CLASS_ORDER,
        digits=4,
        zero_division=0,
    )
    accuracy = float((y_val.values == val_pred).mean())
    selected_feature_count = len(model_results['selected_columns'])

    # Calculate per-class metrics
    class_metrics = {}
    for i, class_name in enumerate(CLASS_ORDER):
        mask = y_val == i
        if mask.sum() > 0:
            class_acc = (val_pred[mask] == i).mean()
            class_metrics[class_name] = f'{class_acc:.4f}'

    summary_lines = [
        '================================================',
        'Random Forest Multi-class Classification',
        'VALIDATION COHORT RESULTS',
        '================================================',
        '',
        'Dataset Information:',
        f'  Training samples: {len(df_train)}',
        f'  Validation samples: {len(df_val)}',
        f'  Selected features: {selected_feature_count}',
        f'  Training groups: {df_train["group"].value_counts().to_dict()}',
        f'  Validation groups: {df_val["group"].value_counts().to_dict()}',
        '',
        'Performance Metrics:',
        f'  Validation accuracy: {accuracy:.4f}',
        '',
        'Per-class Accuracy (Validation):',
        *[f'  {class_name}: {acc}' for class_name, acc in class_metrics.items()],
        '',
        'Label encoding:',
        '  CTRL = 0',
        '  CRC-Well = 1',
        '  CRC-Poor = 2',
        '',
        'Classification report (Validation):',
        report_text,
    ]
    (output_dir / 'validation_summary.txt').write_text('\n'.join(summary_lines), encoding='utf-8')

    print(f'验证结果已保存至: {output_dir}')


def main() -> None:
    sns.set_theme(style='whitegrid', context='talk')

    project_root = Path(__file__).resolve().parents[1]
    output_dir = project_root / 'results' / 'random_forest_validation'
    output_dir.mkdir(parents=True, exist_ok=True)

    # Load and prepare data
    print('正在加载数据...')
    df = load_analysis_data()
    x, y = prepare_features(df)

    # Split into training and validation sets (70-30 split)
    print('正在分离训练集和验证集...')
    x_train, x_val, y_train, y_val, idx_train, idx_val = train_test_split(
        x, y, df.index, test_size=0.3, random_state=42, stratify=y
    )

    df_train = df.loc[idx_train].copy()
    df_val = df.loc[idx_val].copy()

    print(f'训练集: {len(df_train)} 样本')
    print(f'验证集: {len(df_val)} 样本')
    print(f'训练集分组: {df_train["group"].value_counts().to_dict()}')
    print(f'验证集分组: {df_val["group"].value_counts().to_dict()}')

    # Train model on training data
    print('正在训练模型...')
    model_results = train_model(x_train, y_train)

    # Predict on validation set
    print('正在进行验证集预测...')
    x_val_selected = model_results['selector'].transform(x_val)
    val_pred = model_results['forest'].predict(x_val_selected)
    val_proba = model_results['forest'].predict_proba(x_val_selected)

    # Generate plots and reports
    print('正在生成结果图表...')
    plot_feature_importance(model_results['importance_df'], output_dir)
    roc_df = plot_validation_roc_curves(y_val, val_proba, output_dir)
    confusion_df = plot_validation_confusion(y_val, val_pred, output_dir)

    # Save all results
    save_validation_reports(
        output_dir,
        df_train,
        df_val,
        y_val,
        val_pred,
        val_proba,
        model_results,
        roc_df,
        confusion_df,
    )

    print('✅ 验证队列的ROC曲线已生成完毕！')
    print(f'输出目录: {output_dir}')


if __name__ == '__main__':
    main()
